        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if batch_idx % 5000 == 0 and batch_idx != 0:
                    print(epoch, batch_idx, "time:", time.time() - start_time, "loss:",
            ...
            if info[0] >= onset_thres and onset_seq[i] == np.amax(onset_seq[backward_frames: forward_frames]):
                ...
                if current_onset is None:
                    ...
                    if len(pitch_counter) > 0:
                        result.append(
                            [current_onset, current_time, max(set(pitch_counter), key=pitch_counter.count) + 36])
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(test_loader)):
                ...
                for bid, song_id in enumerate(song_ids):
                    ...
                    frames_table.append(frame_info)

            results = self._parse_frame_info(frames_table, onset_thres=onset_thres, offset_thres=offset_thres)
